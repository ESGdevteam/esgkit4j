...
public interface EsgNodeService {
     ...
     * @return The block details, wrapped in a Single
     */
    Single<Block> getBlock(EsgTimestamp timestamp);
     ...
     */
    @Deprecated
    Single<EsgID> getBlockId(int height);
     ...
     * @return The block details, wrapped in a single
     */
    Single<Account> getAccount(EsgAddress accountId);
     ...
     */
    Single<AT[]> getAccountATs(EsgAddress accountId);
     ...
     */
    @Deprecated
    Single<EsgID[]> getAccountBlockIDs(EsgAddress accountId); // TODO timestamp, firstIndex, lastIndex
     ...
     * @return The blocks, wrapped in a single
     */
    Single<Block[]> getAccountBlocks(EsgAddress accountId); // TODO timestamp, firstIndex, lastIndex, includeTransactions
     ...
     */
    Single<EsgID[]> getAccountTransactionIDs(EsgAddress accountId); // TODO filtering
     ...
     */
    Single<Transaction[]> getAccountTransactions(EsgAddress accountId); // TODO filtering
     ...
     */
    Single<EsgAddress[]> getAccountsWithRewardRecipient(EsgAddress accountId); // TODO finish
     ...
     */
    Single<Transaction> getTransaction(EsgID transactionId);
     ...
     */
    Single<byte[]> getTransactionBytes(EsgID transactionId);
     ...
     */
    Single<byte[]> generateTransaction(EsgAddress recipient, byte[] senderPublicKey, EsgValue amount, EsgValue fee, int deadline);
     ...
     * @return The unsigned transaction bytes, wrapped in a single
     */
    Single<byte[]> generateTransactionWithMessage(EsgAddress recipient, byte[] senderPublicKey, EsgValue amount, EsgValue fee, int deadline, String message);
     ...
     * @return The unsigned transaction bytes, wrapped in a single
     */
    Single<byte[]> generateTransactionWithMessage(EsgAddress recipient, byte[] senderPublicKey, EsgValue amount, EsgValue fee, int deadline, byte[] message);
     ...
     * @return The unsigned transaction bytes, wrapped in a single
     */
    Single<byte[]> generateTransactionWithEncryptedMessage(EsgAddress recipient, byte[] senderPublicKey, EsgValue amount, EsgValue fee, int deadline, EsgEncryptedMessage message);
     ...
     * @return The unsigned transaction bytes, wrapped in a single
     */
    Single<byte[]> generateTransactionWithEncryptedMessageToSelf(EsgAddress recipient, byte[] senderPublicKey, EsgValue amount, EsgValue fee, int deadline, EsgEncryptedMessage message);
     ...
     */
    Single<EsgAddress> getRewardRecipient(EsgAddress account);
     ...
     */
    Single<Long> submitNonce(String passphrase, String nonce, EsgID accountId);
     ...
     */
    Single<byte[]> generateMultiOutTransaction(byte[] senderPublicKey, EsgValue fee, int deadline, Map<EsgAddress, EsgValue> recipients) throws IllegalArgumentException;
     ...
     */
    Single<byte[]> generateMultiOutSameTransaction(byte[] senderPublicKey, EsgValue amount, EsgValue fee, int deadline, Set<EsgAddress> recipients) throws IllegalArgumentException;
     ...
     * @return The unsigned transaction bytes, wrapped in a single
     */
    Single<byte[]> generateCreateATTransaction(byte[] senderPublicKey, EsgValue fee, int deadline, String name, String description, byte[] creationBytes);

    static EsgNodeService getInstance(String nodeAddress) {
        return getInstance(nodeAddress, null);
    }

    static EsgNodeService getInstance(String nodeAddress, String httpUserAgent) {
        if (nodeAddress.startsWith("grpc://")) {
            return new GrpcEsgNodeService(nodeAddress, new DefaultSchedulerAssigner());
        } else {
            return new HttpEsgNodeService(nodeAddress, httpUserAgent, new DefaultSchedulerAssigner());
        }
    }

    static EsgNodeService getCompositeInstance(String... nodeAddresses) {
        return getCompositeInstanceWithUserAgent(null, nodeAddresses);
    }

    static EsgNodeService getCompositeInstanceWithUserAgent(String httpUserAgent, String... nodeAddresses) {
        return new CompositeEsgNodeService(Arrays.stream(nodeAddresses)
                .map(nodeAddress -> getInstance(nodeAddress, httpUserAgent))
                .toArray(EsgNodeService[]::new));
    }
}
