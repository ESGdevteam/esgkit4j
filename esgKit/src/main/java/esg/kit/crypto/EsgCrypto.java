...
public interface EsgCrypto {
    ...
    /**
     * Derive the esg address of the passphrase
     * @param passphrase The passphrase
     * @return The esg address
     */
    EsgAddress getEsgAddressFromPassphrase(String passphrase);

    /**
     * Derive the esg address of the private key
     * @param privateKey The private key
     * @return The esg address
     */
    EsgAddress getEsgAddressFromPrivate(byte[] privateKey);

    /**
     * Derive the esg address of the public key
     * @param publicKey The public key
     * @return The esg address
     */
    EsgAddress getEsgAddressFromPublic(byte[] publicKey);

    /**
     * Convert a full hash to an ID
     * @param hash The hash of an account, block, transaction etc
     * @return The Esg ID of the account, block, transaction etc
     * @throws IllegalArgumentException if the hash is invalid
     */
    EsgID hashToId(byte[] hash) throws IllegalArgumentException;
     ...
     * @return The encrypted message
     */
    EsgEncryptedMessage encryptBytesMessage(byte[] message, String myPassphrase, byte[] theirPublicKey);
     ...
     * @return The encrypted message
     */
    EsgEncryptedMessage encryptBytesMessage(byte[] message, byte[] myPrivateKey, byte[] theirPublicKey);
     ...
     * @return The encrypted message
     */
    EsgEncryptedMessage encryptTextMessage(String message, String myPassphrase, byte[] theirPublicKey);
     ...
     * @return The encrypted message
     */
    EsgEncryptedMessage encryptTextMessage(String message, byte[] myPrivateKey, byte[] theirPublicKey);
     ...
     * @return The plaintext message
     */
    byte[] decryptMessage(EsgEncryptedMessage message, String myPassphrase, byte[] theirPublicKey);
     ...
     * @return The plaintext message
     */
    byte[] decryptMessage(EsgEncryptedMessage message, byte[] myPrivateKey, byte[] theirPublicKey);

    /**
     * Get the Reed-Solomon encoding of a Esg address. Does not include the "ESG-" prefix.
     * @param esgID The Esg ID
     * @return The Reed-Solomon encoding of the address, not including the "ESG-" prefix.
     */
    String rsEncode(EsgID esgID);

    /**
     * Get the account ID associated with a Reed-Solomon encoded address
     * @param rs The Reed-Solomon encoded address, not including the "ESG-" prefix
     * @return The ID of that RS address
     * @throws IllegalArgumentException If the RS address was invalid and could not be decoded.
     */
    EsgID rsDecode(String rs) throws IllegalArgumentException;

    /**
     * Translate an epoch time to a Java Date object
     * @param epochTime The number of seconds since the Esg epoch
     ...
     * @return The generation signature of the block
     */
    byte[] calculateGenerationSignature(byte[] lastGenSig, EsgAddress lastGenerator);
     ...
     * @return The hit of that scoop
     */
    BigInteger calculateHit(EsgAddress accountId, long nonce, byte[] genSig, int scoop, int pocVersion);
     ...
     * @return The hit of that scoop
     */
    BigInteger calculateHit(EsgAddress accountId, long nonce, byte[] genSig, byte[] scoopData);
     ...
     * @return The hit of that scoop
     */
    BigInteger calculateDeadline(EsgAddress accountId, long nonce, byte[] genSig, int scoop, long baseTarget, int pocVersion);
     ...
     * @return
     */
    byte[] getATCreationBytes(short atVersion, byte[] code, byte[] data, int dPages, int csPages, int usPages, EsgValue minActivationAmount);

    /**
     * Get a singleton instance of this class
     * @return Singleton instance of this class.
     */
    static EsgCrypto getInstance() {
        return EsgCryptoImpl.INSTANCE;
    }
}
