...
import esg.kit.crypto.ec.Curve25519;
import esg.kit.crypto.ec.Curve25519Impl;
import esg.kit.crypto.hash.EsgHashProvider;
import esg.kit.crypto.hash.shabal.Shabal256;
import esg.kit.crypto.plot.PlotCalculator;
import esg.kit.crypto.plot.impl.PlotCalculatorImpl;
import esg.kit.crypto.rs.ReedSolomon;
import esg.kit.crypto.rs.ReedSolomonImpl;
...
class EsgCryptoImpl extends AbstractEsgCrypto {

    static final EsgCryptoImpl INSTANCE = new EsgCryptoImpl();
    ...
    @Override
    public EsgAddress getEsgAddressFromPublic(byte[] publicKey) {
        return EsgAddress.fromId(hashToId(getSha256().digest(publicKey)));
    }

    @Override
    public EsgID hashToId(byte[] hash) throws IllegalArgumentException {
        if (hash == null || hash.length < 8) {
            throw new IllegalArgumentException("Invalid hash: " + Arrays.toString(hash));
        }
        return EsgID.fromLong(bytesToLong(hash));
    }
    ...
    @Override
    public EsgEncryptedMessage encryptBytesMessage(byte[] message, byte[] myPrivateKey, byte[] theirPublicKey) {
        return encryptPlainMessage(message, false, myPrivateKey, theirPublicKey);
    }

    @Override
    public EsgEncryptedMessage encryptTextMessage(String message, byte[] myPrivateKey, byte[] theirPublicKey) {
        return encryptPlainMessage(stringToBytes(message), true, myPrivateKey, theirPublicKey);
    }

    private EsgEncryptedMessage encryptPlainMessage(byte[] message, boolean isText, byte[] myPrivateKey, byte[] theirPublicKey) {
        if (message.length == 0) {
            return new EsgEncryptedMessage(new byte[0], new byte[0], isText);
        }
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream(); GZIPOutputStream gzip = new GZIPOutputStream(bos)) {
            ...
            return new EsgEncryptedMessage(data, nonce, isText);
        } catch (IOException e) {
            throw new RuntimeException(e.getMessage(), e);
        }
    }
    ...
    @Override
    public byte[] decryptMessage(EsgEncryptedMessage message, byte[] myPrivateKey, byte[] theirPublicKey) {
    ...
    @Override
    public String rsEncode(EsgID esgID) {
        return reedSolomon.encode(esgID.getSignedLongId());
    }

    @Override
    public EsgID rsDecode(String rs) throws IllegalArgumentException {
        rs = rs.toUpperCase();
        long rsValue;
        try {
    ...
    @Override
    public byte[] getATCreationBytes(short atVersion, byte[] code, byte[] data, int dPages, int csPages, int usPages, EsgValue minActivationAmount) {
