...
abstract class AbstractEsgCrypto implements EsgCrypto {
    ...
    @Override
    public EsgAddress getEsgAddressFromPassphrase(String passphrase) {
        return getEsgAddressFromPrivate(getPrivateKey(passphrase));
    }

    @Override
    public EsgAddress getEsgAddressFromPrivate(byte[] privateKey) {
        return getEsgAddressFromPublic(getPublicKey(privateKey));
    }
    ...
    @Override
    public EsgEncryptedMessage encryptBytesMessage(byte[] message, String myPassphrase, byte[] theirPublicKey) {
        return encryptBytesMessage(message, getPrivateKey(myPassphrase), theirPublicKey);
    }

    @Override
    public EsgEncryptedMessage encryptTextMessage(String message, String myPassphrase, byte[] theirPublicKey) {
        return encryptTextMessage(message, getPrivateKey(myPassphrase), theirPublicKey);
    }

    @Override
    public byte[] decryptMessage(EsgEncryptedMessage message, String myPassphrase, byte[] theirPublicKey) {
        return decryptMessage(message, getPrivateKey(myPassphrase), theirPublicKey);
    }

    @Override
    public byte[] calculateGenerationSignature(byte[] lastGenSig, EsgAddress lastGenerator) {
        return calculateGenerationSignature(lastGenSig, lastGenerator.getSignedLongId());
    }

    @Override
    public BigInteger calculateHit(EsgAddress accountId, long nonce, byte[] genSig, int scoop, int pocVersion) {
        return calculateHit(accountId.getSignedLongId(), nonce, genSig, scoop, pocVersion);
    }

    @Override
    public BigInteger calculateHit(EsgAddress accountId, long nonce, byte[] genSig, byte[] scoopData) {
        return calculateHit(accountId.getSignedLongId(), nonce, genSig, scoopData);
    }

    @Override
    public BigInteger calculateDeadline(EsgAddress accountId, long nonce, byte[] genSig, int scoop, long baseTarget, int pocVersion) {
