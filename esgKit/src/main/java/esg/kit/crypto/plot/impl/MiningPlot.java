package esg.kit.crypto.plot.impl;
...
class MiningPlot {
    ...
    MiningPlot(Supplier<MessageDigest> shabal256Supplier, long addr, long nonce, int pocVersion) {
        System.arraycopy(EsgCrypto.getInstance().longToBytes(addr), 0, data, PLOT_SIZE, 8);
        System.arraycopy(EsgCrypto.getInstance().longToBytes(nonce), 0, data, PLOT_SIZE + 8, 8);
