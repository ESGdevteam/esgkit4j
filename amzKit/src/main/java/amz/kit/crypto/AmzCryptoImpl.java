...
import amz.kit.crypto.ec.Curve25519;
import amz.kit.crypto.ec.Curve25519Impl;
import amz.kit.crypto.hash.AmzHashProvider;
import amz.kit.crypto.hash.shabal.Shabal256;
import amz.kit.crypto.plot.PlotCalculator;
import amz.kit.crypto.plot.impl.PlotCalculatorImpl;
import amz.kit.crypto.rs.ReedSolomon;
import amz.kit.crypto.rs.ReedSolomonImpl;
...
class AmzCryptoImpl extends AbstractAmzCrypto {

    static final AmzCryptoImpl INSTANCE = new AmzCryptoImpl();
    ...
    @Override
    public AmzAddress getAmzAddressFromPublic(byte[] publicKey) {
        return AmzAddress.fromId(hashToId(getSha256().digest(publicKey)));
    }

    @Override
    public AmzID hashToId(byte[] hash) throws IllegalArgumentException {
        if (hash == null || hash.length < 8) {
            throw new IllegalArgumentException("Invalid hash: " + Arrays.toString(hash));
        }
        return AmzID.fromLong(bytesToLong(hash));
    }
    ...
    @Override
    public AmzEncryptedMessage encryptBytesMessage(byte[] message, byte[] myPrivateKey, byte[] theirPublicKey) {
        return encryptPlainMessage(message, false, myPrivateKey, theirPublicKey);
    }

    @Override
    public AmzEncryptedMessage encryptTextMessage(String message, byte[] myPrivateKey, byte[] theirPublicKey) {
        return encryptPlainMessage(stringToBytes(message), true, myPrivateKey, theirPublicKey);
    }

    private AmzEncryptedMessage encryptPlainMessage(byte[] message, boolean isText, byte[] myPrivateKey, byte[] theirPublicKey) {
        if (message.length == 0) {
            return new AmzEncryptedMessage(new byte[0], new byte[0], isText);
        }
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream(); GZIPOutputStream gzip = new GZIPOutputStream(bos)) {
            ...
            return new AmzEncryptedMessage(data, nonce, isText);
        } catch (IOException e) {
            throw new RuntimeException(e.getMessage(), e);
        }
    }
    ...
    @Override
    public byte[] decryptMessage(AmzEncryptedMessage message, byte[] myPrivateKey, byte[] theirPublicKey) {
    ...
    @Override
    public String rsEncode(AmzID amzID) {
        return reedSolomon.encode(amzID.getSignedLongId());
    }

    @Override
    public AmzID rsDecode(String rs) throws IllegalArgumentException {
        rs = rs.toUpperCase();
        long rsValue;
        try {
    ...
    @Override
    public byte[] getATCreationBytes(short atVersion, byte[] code, byte[] data, int dPages, int csPages, int usPages, AmzValue minActivationAmount) {
