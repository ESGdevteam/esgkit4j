package amz.kit.crypto.plot.impl;
...
class MiningPlot {
    ...
    MiningPlot(Supplier<MessageDigest> shabal256Supplier, long addr, long nonce, int pocVersion) {
        System.arraycopy(AmzCrypto.getInstance().longToBytes(addr), 0, data, PLOT_SIZE, 8);
        System.arraycopy(AmzCrypto.getInstance().longToBytes(nonce), 0, data, PLOT_SIZE + 8, 8);
