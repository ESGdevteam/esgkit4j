...
abstract class AbstractAmzCrypto implements AmzCrypto {
    ...
    @Override
    public AmzAddress getAmzAddressFromPassphrase(String passphrase) {
        return getAmzAddressFromPrivate(getPrivateKey(passphrase));
    }

    @Override
    public AmzAddress getAmzAddressFromPrivate(byte[] privateKey) {
        return getAmzAddressFromPublic(getPublicKey(privateKey));
    }
    ...
    @Override
    public AmzEncryptedMessage encryptBytesMessage(byte[] message, String myPassphrase, byte[] theirPublicKey) {
        return encryptBytesMessage(message, getPrivateKey(myPassphrase), theirPublicKey);
    }

    @Override
    public AmzEncryptedMessage encryptTextMessage(String message, String myPassphrase, byte[] theirPublicKey) {
        return encryptTextMessage(message, getPrivateKey(myPassphrase), theirPublicKey);
    }

    @Override
    public byte[] decryptMessage(AmzEncryptedMessage message, String myPassphrase, byte[] theirPublicKey) {
        return decryptMessage(message, getPrivateKey(myPassphrase), theirPublicKey);
    }

    @Override
    public byte[] calculateGenerationSignature(byte[] lastGenSig, AmzAddress lastGenerator) {
        return calculateGenerationSignature(lastGenSig, lastGenerator.getSignedLongId());
    }

    @Override
    public BigInteger calculateHit(AmzAddress accountId, long nonce, byte[] genSig, int scoop, int pocVersion) {
        return calculateHit(accountId.getSignedLongId(), nonce, genSig, scoop, pocVersion);
    }

    @Override
    public BigInteger calculateHit(AmzAddress accountId, long nonce, byte[] genSig, byte[] scoopData) {
        return calculateHit(accountId.getSignedLongId(), nonce, genSig, scoopData);
    }

    @Override
    public BigInteger calculateDeadline(AmzAddress accountId, long nonce, byte[] genSig, int scoop, long baseTarget, int pocVersion) {
