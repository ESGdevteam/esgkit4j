...
public interface AmzCrypto {
    ...
    /**
     * Derive the amz address of the passphrase
     * @param passphrase The passphrase
     * @return The amz address
     */
    AmzAddress getAmzAddressFromPassphrase(String passphrase);

    /**
     * Derive the amz address of the private key
     * @param privateKey The private key
     * @return The amz address
     */
    AmzAddress getAmzAddressFromPrivate(byte[] privateKey);

    /**
     * Derive the amz address of the public key
     * @param publicKey The public key
     * @return The amz address
     */
    AmzAddress getAmzAddressFromPublic(byte[] publicKey);

    /**
     * Convert a full hash to an ID
     * @param hash The hash of an account, block, transaction etc
     * @return The Amz ID of the account, block, transaction etc
     * @throws IllegalArgumentException if the hash is invalid
     */
    AmzID hashToId(byte[] hash) throws IllegalArgumentException;
     ...
     * @return The encrypted message
     */
    AmzEncryptedMessage encryptBytesMessage(byte[] message, String myPassphrase, byte[] theirPublicKey);
     ...
     * @return The encrypted message
     */
    AmzEncryptedMessage encryptBytesMessage(byte[] message, byte[] myPrivateKey, byte[] theirPublicKey);
     ...
     * @return The encrypted message
     */
    AmzEncryptedMessage encryptTextMessage(String message, String myPassphrase, byte[] theirPublicKey);
     ...
     * @return The encrypted message
     */
    AmzEncryptedMessage encryptTextMessage(String message, byte[] myPrivateKey, byte[] theirPublicKey);
     ...
     * @return The plaintext message
     */
    byte[] decryptMessage(AmzEncryptedMessage message, String myPassphrase, byte[] theirPublicKey);
     ...
     * @return The plaintext message
     */
    byte[] decryptMessage(AmzEncryptedMessage message, byte[] myPrivateKey, byte[] theirPublicKey);

    /**
     * Get the Reed-Solomon encoding of a Amz address. Does not include the "AMZ-" prefix.
     * @param amzID The Amz ID
     * @return The Reed-Solomon encoding of the address, not including the "AMZ-" prefix.
     */
    String rsEncode(AmzID amzID);

    /**
     * Get the account ID associated with a Reed-Solomon encoded address
     * @param rs The Reed-Solomon encoded address, not including the "AMZ-" prefix
     * @return The ID of that RS address
     * @throws IllegalArgumentException If the RS address was invalid and could not be decoded.
     */
    AmzID rsDecode(String rs) throws IllegalArgumentException;

    /**
     * Translate an epoch time to a Java Date object
     * @param epochTime The number of seconds since the Amz epoch
     ...
     * @return The generation signature of the block
     */
    byte[] calculateGenerationSignature(byte[] lastGenSig, AmzAddress lastGenerator);
     ...
     * @return The hit of that scoop
     */
    BigInteger calculateHit(AmzAddress accountId, long nonce, byte[] genSig, int scoop, int pocVersion);
     ...
     * @return The hit of that scoop
     */
    BigInteger calculateHit(AmzAddress accountId, long nonce, byte[] genSig, byte[] scoopData);
     ...
     * @return The hit of that scoop
     */
    BigInteger calculateDeadline(AmzAddress accountId, long nonce, byte[] genSig, int scoop, long baseTarget, int pocVersion);
     ...
     * @return
     */
    byte[] getATCreationBytes(short atVersion, byte[] code, byte[] data, int dPages, int csPages, int usPages, AmzValue minActivationAmount);

    /**
     * Get a singleton instance of this class
     * @return Singleton instance of this class.
     */
    static AmzCrypto getInstance() {
        return AmzCryptoImpl.INSTANCE;
    }
}
