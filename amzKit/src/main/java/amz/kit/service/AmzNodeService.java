...
public interface AmzNodeService {
     ...
     * @return The block details, wrapped in a Single
     */
    Single<Block> getBlock(AmzTimestamp timestamp);
     ...
     */
    @Deprecated
    Single<AmzID> getBlockId(int height);
     ...
     * @return The block details, wrapped in a single
     */
    Single<Account> getAccount(AmzAddress accountId);
     ...
     */
    Single<AT[]> getAccountATs(AmzAddress accountId);
     ...
     */
    @Deprecated
    Single<AmzID[]> getAccountBlockIDs(AmzAddress accountId); // TODO timestamp, firstIndex, lastIndex
     ...
     * @return The blocks, wrapped in a single
     */
    Single<Block[]> getAccountBlocks(AmzAddress accountId); // TODO timestamp, firstIndex, lastIndex, includeTransactions
     ...
     */
    Single<AmzID[]> getAccountTransactionIDs(AmzAddress accountId); // TODO filtering
     ...
     */
    Single<Transaction[]> getAccountTransactions(AmzAddress accountId); // TODO filtering
     ...
     */
    Single<AmzAddress[]> getAccountsWithRewardRecipient(AmzAddress accountId); // TODO finish
     ...
     */
    Single<Transaction> getTransaction(AmzID transactionId);
     ...
     */
    Single<byte[]> getTransactionBytes(AmzID transactionId);
     ...
     */
    Single<byte[]> generateTransaction(AmzAddress recipient, byte[] senderPublicKey, AmzValue amount, AmzValue fee, int deadline);
     ...
     * @return The unsigned transaction bytes, wrapped in a single
     */
    Single<byte[]> generateTransactionWithMessage(AmzAddress recipient, byte[] senderPublicKey, AmzValue amount, AmzValue fee, int deadline, String message);
     ...
     * @return The unsigned transaction bytes, wrapped in a single
     */
    Single<byte[]> generateTransactionWithMessage(AmzAddress recipient, byte[] senderPublicKey, AmzValue amount, AmzValue fee, int deadline, byte[] message);
     ...
     * @return The unsigned transaction bytes, wrapped in a single
     */
    Single<byte[]> generateTransactionWithEncryptedMessage(AmzAddress recipient, byte[] senderPublicKey, AmzValue amount, AmzValue fee, int deadline, AmzEncryptedMessage message);
     ...
     * @return The unsigned transaction bytes, wrapped in a single
     */
    Single<byte[]> generateTransactionWithEncryptedMessageToSelf(AmzAddress recipient, byte[] senderPublicKey, AmzValue amount, AmzValue fee, int deadline, AmzEncryptedMessage message);
     ...
     */
    Single<AmzAddress> getRewardRecipient(AmzAddress account);
     ...
     */
    Single<Long> submitNonce(String passphrase, String nonce, AmzID accountId);
     ...
     */
    Single<byte[]> generateMultiOutTransaction(byte[] senderPublicKey, AmzValue fee, int deadline, Map<AmzAddress, AmzValue> recipients) throws IllegalArgumentException;
     ...
     */
    Single<byte[]> generateMultiOutSameTransaction(byte[] senderPublicKey, AmzValue amount, AmzValue fee, int deadline, Set<AmzAddress> recipients) throws IllegalArgumentException;
     ...
     * @return The unsigned transaction bytes, wrapped in a single
     */
    Single<byte[]> generateCreateATTransaction(byte[] senderPublicKey, AmzValue fee, int deadline, String name, String description, byte[] creationBytes);

    static AmzNodeService getInstance(String nodeAddress) {
        return getInstance(nodeAddress, null);
    }

    static AmzNodeService getInstance(String nodeAddress, String httpUserAgent) {
        if (nodeAddress.startsWith("grpc://")) {
            return new GrpcAmzNodeService(nodeAddress, new DefaultSchedulerAssigner());
        } else {
            return new HttpAmzNodeService(nodeAddress, httpUserAgent, new DefaultSchedulerAssigner());
        }
    }

    static AmzNodeService getCompositeInstance(String... nodeAddresses) {
        return getCompositeInstanceWithUserAgent(null, nodeAddresses);
    }

    static AmzNodeService getCompositeInstanceWithUserAgent(String httpUserAgent, String... nodeAddresses) {
        return new CompositeAmzNodeService(Arrays.stream(nodeAddresses)
                .map(nodeAddress -> getInstance(nodeAddress, httpUserAgent))
                .toArray(AmzNodeService[]::new));
    }
}
